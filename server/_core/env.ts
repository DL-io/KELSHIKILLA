...
export const ENV = {
  ...
  // Legacy forge fallback (optional)
  forgeApiUrl: process.env.FORGE_API_URL ?? "",
  forgeApiKey: process.env.FORGE_API_KEY ?? "",

  // LLM fallback provider list (comma separated): openrouter,grok,forge
  llmFallbackProviders: process.env.LLM_FALLBACK_PROVIDERS ?? "",

  // Arbs.xyz upstream (optional)
  arbsXyzBaseUrl: process.env.ARBS_XYZ_BASE_URL ?? "",
  ...
  polymarketDefaultTickSize: Number(
    process.env.POLYMARKET_DEFAULT_TICK_SIZE ?? "0.01"
  ),
  ...
  killswitchNotionalCapUsd: Number(
    process.env.KILLSWITCH_NOTIONAL_CAP_USD ?? "100"
  ),
  killswitchOrdersPerMin: Number(process.env.KILLSWITCH_ORDERS_PER_MIN ?? "6"),
  killswitchPerMarketCapUsd: Number(
    process.env.KILLSWITCH_PER_MARKET_CAP_USD ?? "100"
  ),
  killswitchMaxSpreadBps: Number(
    process.env.KILLSWITCH_MAX_SPREAD_BPS ?? "500"
  ),

  // Aliases used by polymarket adapter/killswitch (same semantic values)
  polymarketMaxNotionalUsd: Number(
    process.env.KILLSWITCH_NOTIONAL_CAP_USD ?? "100"
  ),
  polymarketMaxOrdersPerMinute: Number(
    process.env.KILLSWITCH_ORDERS_PER_MIN ?? "6"
  ),
...
export function validateProductionEnv(): void {
  ...
  if (
    ...
    missing.push(
      "At least one LLM key: OPENAI_API_KEY, ANTHROPIC_API_KEY, GROQ_API_KEY, or GROK_API_KEY"
    );
  }
...
export function getLiveReadinessReport(): {
  ...
  if (!ENV.redisUrl) {
    warnings.push(
      "REDIS_URL not set — strategy refinement and memory disabled"
    );
  }
