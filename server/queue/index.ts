...
export function getTradingQueue(): Queue | null {
  return (_tradingQueue ??= makeQueue(QUEUES.TRADING));
}
export function getRefinementQueue(): Queue | null {
  return (_refinementQueue ??= makeQueue(QUEUES.REFINEMENT));
}
export function getReportingQueue(): Queue | null {
  return (_reportingQueue ??= makeQueue(QUEUES.REPORTING));
}
export function getMemoryQueue(): Queue | null {
  return (_memoryQueue ??= makeQueue(QUEUES.MEMORY));
}
export function getLifecycleQueue(): Queue | null {
  return (_lifecycleQueue ??= makeQueue(QUEUES.LIFECYCLE));
}
...
// Typed helpers
export const addTradingTask = (name: string, data: Record<string, unknown>) =>
  enqueue(getTradingQueue, name, data);
export const addRefinementTask = (
  name: string,
  data: Record<string, unknown>
) => enqueue(getRefinementQueue, name, data);
export const addReportingTask = (name: string, data: Record<string, unknown>) =>
  enqueue(getReportingQueue, name, data);
export const addMemoryTask = (name: string, data: Record<string, unknown>) =>
  enqueue(getMemoryQueue, name, data);
export const addLifecycleTask = (name: string, data: Record<string, unknown>) =>
  enqueue(getLifecycleQueue, name, data);
