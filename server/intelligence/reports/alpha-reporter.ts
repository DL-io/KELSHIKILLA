...
export async function generateAlphaReport(): Promise<AlphaReport> {
  ...
  if (!db) {
    return {
      generatedAt:     now,
      summary:         { trades24h: 0, winRate24h: 0, realizedPnlUsd: 0, avgEdge: 0, avgConfidence: 0, alertLevel: "yellow" },
      categoryAlpha:   [],
  ...
  const raw = ((Array.isArray(tradeRows) ? tradeRows[0] : tradeRows) as any[])[0] ?? {};
  ...
  const catRawRows = (Array.isArray(catRows) ? catRows[0] : catRows) as any[];
  ...
  let memoryStats: MemoryStats = { totalEvents: 0, causalEvents: 0, recentMatches: 0 };
  try {
    const memRows = await db.execute(sql`
      ...
      FROM vector_memory
    `);
    const memRaw = ((Array.isArray(memRows) ? memRows[0] : memRows) as any[])[0] ?? {};
    memoryStats = {
  ...
  const alertLevel: "green" | "yellow" | "red" =
    realizedPnlUsd < -50   ? "red"
    : winRate24h < 0.40    ? "yellow"
    : "green";

  // ─── Recommendations ─────────────────────────────────────────────────────

  const recommendations: string[] = [];
  if (trades24h === 0)             recommendations.push("No trades in 24h — check market scanner, spread filters, or edge threshold");
  if (winRate24h < 0.45 && trades24h > 5) recommendations.push(`Win rate ${(winRate24h * 100).toFixed(1)}% — consider raising minConfidence or minEdge`);
  if (avgEdge < 0.06 && trades24h > 5)    recommendations.push(`Average edge ${(avgEdge * 100).toFixed(1)}% is low — tighten edge threshold`);
  if (memoryStats.totalEvents < 10)        recommendations.push("Vector memory has <10 events — run more paper trades before live");
  if (recommendations.length === 0)        recommendations.push("All metrics nominal — system performing as expected");
